 ...
 */
@Slf4j
public final class SelectorThread<T extends MMOClient<?>> extends Thread implements BuffersHolder {
    // default BYTE_ORDER
    ...
    // Implementations
    private final IPacketHandler<L2LoginClient, L2LoginClientPacket> _packetHandler;
    ...
    // Pending Close
    private final NioNetStackList<MMOConnection> _pendingClose;
    private final LoginController loginController;
    private final LoginPacketsProcessor packetsProcessor;
    public boolean TCP_NODELAY;
    private boolean _shutdown;

    public SelectorThread(final LoginController loginController,
                          final LoginPacketsProcessor packetsProcessor,
                          final SelectorConfig sc,
                          final IMMOExecutor executor,
                          final IPacketHandler<L2LoginClient, L2LoginClientPacket> packetHandler,
    ...
    private void openServerSocket(InetAddress address, int tcpPort) {
        try {
            ...
            ss.bind(resolveSocketAddress(address, tcpPort));
    ...
    private InetSocketAddress resolveSocketAddress(InetAddress address, int tcpPort) {
        return address == null ? new InetSocketAddress(tcpPort) : new InetSocketAddress(address, tcpPort);
    }
    ...
    public final void recycleBuffer(ByteBuffer buf) {
    ...
    private final void finishConnection(final SelectionKey key, final MMOConnection con) {
        try {
    ...
    private final void acceptConnection(final SelectionKey key, MMOConnection con) {
        ...
        try {
            while ((sc = ssc.accept()) != null) {
                if ((_acceptFilter == null) || _acceptFilter.accept(sc)) {
                    ...
                    con = new MMOConnection<L2LoginClient>(this, sc.socket(), clientKey, TCP_NODELAY);
                    con.setClient(_clientFactory.create(con, loginController.getScrambledRSAKeyPair(), loginController.getBlowfishKey()));
    ...
    private final void readPacket(final SelectionKey key, final MMOConnection<L2LoginClient> con) {
        if (!con.isClosed()) {
            ByteBuffer buf;
            ...
            if (result > 0) {
                ...
                final L2LoginClient client = con.getClient();
    ...
    private final boolean tryReadPacket(final SelectionKey key, final L2LoginClient client, final ByteBuffer buf, final MMOConnection con) {
    ...
    private final void allocateReadBuffer(final MMOConnection con) {
        con.setReadBuffer(getPooledBuffer().put(READ_BUFFER));
        READ_BUFFER.clear();
    }

    private final void parseClientPacket(final int pos, final ByteBuffer buf, final int dataSize, final L2LoginClient client) {
        ...
        if (ret && buf.hasRemaining()) {
            ...
            final L2LoginClientPacket cp = _packetHandler.handlePacket(buf, client);
            ...
            if (cp != null) {
                ...
                if (cp.read()) {
                    cp.process(packetsProcessor, client);
    ...
    private final void writeClosePacket(final MMOConnection con) {
        SendablePacket sp;
        synchronized (con.getSendQueue()) {
            ...
            NioNetStackList<SendablePacket> sendQueue = con.getSendQueue();
    ...
    protected final void writePacket(final SelectionKey key, final MMOConnection con) {
    ...
    private final boolean prepareWriteBuffer(final MMOConnection con) {
        ...
        if ((DIRECT_WRITE_BUFFER.remaining() > 1) && !con.hasPendingWriteBuffer()) {
            final NioNetStackList<SendablePacket> sendQueue = con.getSendQueue();
    ...
    private final void closeConnectionImpl(final SelectionKey key, final MMOConnection con) {
        try {
            // notify connection
