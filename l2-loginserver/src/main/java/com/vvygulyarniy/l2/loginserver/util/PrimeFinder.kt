...
/**
 * **Modified for Trove to use the java.util.Arrays sort/search<br></br>
 * algorithms instead of those provided with colt.**<br></br>
 * Used to keep hash table capacities prime numbers.<br></br>
 * Not of interest for users; only for implementors of hashtables.<br></br>
 *
 *
 * Choosing prime numbers as hash table capacities is a good idea<br></br>
 * to keep them working fast, particularly under hash table expansions.<br></br>
 *
 *
 * However, JDK 1.2, JGL 3.1 and many other toolkits do nothing to keep capacities prime.<br></br>
 * This class provides efficient means to choose prime capacities.
 *
 *
 * Choosing a prime is <tt>O(log 300)</tt> (binary search in a list of 300 ints).<br></br>
 * Memory requirements: 1 KB static memory.<br></br>
...
object PrimeFinder {
    ...
    /**
     * The prime number list consists of 11 chunks.<br></br>
     * Each chunk contains prime numbers.<br></br>
     * A chunk starts with a prime P1.<br></br>
     * The next element is a prime P2.<br></br>
     * P2 is the smallest prime for which holds: P2 >= 2*P1.<br></br>
     * The next element is P3, for which the same holds with respect to P2, and so on. Chunks are chosen such that for any desired capacity >= 1000<br></br>
     * the list includes a prime number <= desired capacity * 1.11.<br></br>
     * Therefore, primes can be retrieved which are quite close to any<br></br>
     * desired capacity, which in turn avoids wasting memory.<br></br>
     * For example, the list includes<br></br>
     * 1039,1117,1201,1277,1361,1439,1523,1597,1759,1907,2081.<br></br>
     * So if you need a prime >= 1040, you will find a prime <= 1040*1.11=1154.<br></br>
     * Chunks are chosen such that they are optimized for a hashtable growthfactor of 2.0;<br></br>
     * If your hashtable has such a growthfactor then, after initially<br></br>
     * "rounding to a prime" upon hashtable construction, it will<br></br>
     * later expand to prime capacities such that there exist no better primes.<br></br>
     * In total these are about 32*10=320 numbers -> 1 KB of static memory needed.<br></br>
