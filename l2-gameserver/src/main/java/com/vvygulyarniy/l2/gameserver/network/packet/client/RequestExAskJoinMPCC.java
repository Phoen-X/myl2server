...
public final class RequestExAskJoinMPCC extends L2GameClientPacket {
    ...
    public RequestExAskJoinMPCC(ByteBuffer buf) {
        super(buf);
    }
    ...
    protected void runImpl() {
        ...
        if (activeChar.isInParty()) {
            ...
            if (activeParty.getLeader().equals(activeChar)) {
                ...
                activeChar.send(SystemMessageId.CANNOT_INVITE_TO_COMMAND_CHANNEL);
            }
        }
    }
*/
...
/*
    private void askJoinMPCC(L2PcInstance requestor, L2PcInstance target) {
        ...
        if (!hasRight) {
            requestor.send(SystemMessageId.COMMAND_CHANNEL_ONLY_BY_LEVEL_5_CLAN_LEADER_PARTY_LEADER);
            return;
        }
        ...
        if (!targetLeader.isProcessingRequest()) {
            ...
            targetLeader.send(sm);
            targetLeader.send(new ExAskJoinMPCC(requestor.getName()));
