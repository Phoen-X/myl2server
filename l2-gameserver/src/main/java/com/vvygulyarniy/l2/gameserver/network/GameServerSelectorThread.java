 ...
 */
@Slf4j
public class GameServerSelectorThread extends Thread implements BuffersHolder {
    // default BYTE_ORDER
    ...
    private final int HELPER_BUFFER_SIZE = 65536;
    private final int MAX_SEND_PER_PASS = 10;
    private final int MAX_READ_PER_PASS = 10;
    private final boolean TCP_NODELAY = false;
    // Main Buffers
    ...
    private void openServerSocket(InetAddress address, int tcpPort) {
        try {
            ...
            InetSocketAddress endpoint = resolveSocketAddress(address, tcpPort);
            ss.bind(endpoint);
    ...
    public final void run() {
        openServerSocket(null, 9999);

        int selectedKeysCount = 0;

        SelectionKey key;
        MMOConnection<L2GameClient> con;
        ...
        while (!_shutdown) {
            ...
            if (selectedKeysCount > 0) {
                ...
                while (selectedKeys.hasNext()) {
                    ...
                    con = (MMOConnection<L2GameClient>) key.attachment();
                    switch (key.readyOps()) {
    ...
    private final void finishConnection(final SelectionKey key, final MMOConnection<L2GameClient> con) {
        try {
    ...
    private final void acceptConnection(final SelectionKey key, MMOConnection<L2GameClient> con) {
    ...
    private final void readPacket(final SelectionKey key, final MMOConnection<L2GameClient> con) {
        if (!con.isClosed()) {
            ByteBuffer buf;
            ...
            if (result > 0) {
                ...
                final L2GameClient client = con.getClient();
    ...
    private final boolean tryReadPacket(final SelectionKey key, final L2GameClient client, final ByteBuffer buf, final MMOConnection<L2GameClient> con) {
    ...
    private final void allocateReadBuffer(final MMOConnection<L2GameClient> con) {
        con.setReadBuffer(getPooledBuffer().put(READ_BUFFER));
        READ_BUFFER.clear();
    }

    private final void parseClientPacket(final int pos, final ByteBuffer buf, final int dataSize, final L2GameClient client) {
        ...
        if (ret && buf.hasRemaining()) {
            ...
            final L2GameClientPacket cp = _packetHandler.handlePacket(buf, client);
            ...
            if (cp != null) {
                ...
                //TODO process packet
                cp.process(packetProcessor, client);
                /*if (cp.read()) {
    ...
    private final void writeClosePacket(final MMOConnection<L2GameClient> con) {
        SendablePacket sp;
    ...
    protected final void writePacket(final SelectionKey key, final MMOConnection<L2GameClient> con) {
    ...
    private final boolean prepareWriteBuffer(final MMOConnection<L2GameClient> con) {
    ...
    private final void closeConnectionImpl(final SelectionKey key, final MMOConnection<L2GameClient> con) {
        try {
            // notify connection
