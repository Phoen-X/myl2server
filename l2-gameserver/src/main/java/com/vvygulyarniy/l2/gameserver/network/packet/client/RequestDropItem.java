...
public final class RequestDropItem extends L2GameClientPacket {
    ...
    public RequestDropItem(ByteBuffer buf) {
        super(buf);
    }
    ...
    protected void runImpl() {
        ...
        if (!activeChar.getAccessLevel().allowTransaction()) {
            ...
            activeChar.send(SystemMessageId.NOTHING_HAPPENED);
            return;
        }
        ...
        if (activeChar.isFishing()) {
            ...
            activeChar.send(SystemMessageId.CANNOT_DO_WHILE_FISHING_2);
            return;
        }
        ...
        if ((L2Item.TYPE2_QUEST == item.getItem().getType2()) && !activeChar.canOverrideCond(PcCondOverride.DROP_ALL_ITEMS)) {
            ...
            activeChar.send(SystemMessageId.CANNOT_DISCARD_EXCHANGE_ITEM);
            return;
        }
        ...
        if (!activeChar.isInsideRadius(_x, _y, 0, 150, false, false) || (Math.abs(_z - activeChar.getZ()) > 50)) {
            ...
            activeChar.send(SystemMessageId.CANNOT_DISCARD_DISTANCE_TOO_FAR);
            return;
        }
